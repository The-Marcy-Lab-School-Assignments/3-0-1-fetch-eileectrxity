...
describe(testSuiteName, () => {
  it("app - renders the right status", async () => {
    ...
    expect(builtApp.querySelector("#status-heading").textContent).toBe(
      `Info on - https://jsonplaceholder.typicode.com/users`,
    );
    expect(builtApp.querySelector("#status-code").textContent).toBe(
      `Status code: 200, OK!`,
    );
  ...
  it("app - renders the right users", async () => {
    ...
    liTags.forEach((liTag, i) => {
      ...
      expect(liTag.children[0].textContent).toBe(
        `Load ${users[i].username}'s posts`,
      );
